...
from langchain_community.document_loaders import RRSFeedLoader
...
def main():
    st.title("Preguntas al documento ")  # Establece el título de la aplicación

    loader = st.file_uploader("Sube tu archivo PDF",
                              type="urls")  # Crea un cargador de archivos para subir archivos PDF

    if loader is not None:
        loader_reader = RSSFeedLoader(urls)

        # Almacena el texto del PDF en una variable
        text = ""

        for page in RSSFeedLoader.pages:
